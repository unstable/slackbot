...
response = urllib.request.urlopen('http://hq.sinajs.cn/list=sh000001')
html = response.read()
stock_list = str(html, encoding = "gb2312").replace(";\n",'').replace("var hq_str_",'').replace("=",',').replace('"','').split(",")
content = '上证指数: %s, 今日开盘: %s, 昨日收盘: %s, 当前: %s, 最高: %s, 最低: %s, TIME: %s' % (stock_list[0], stock_list[2], stock_list[3], stock_list[4], stock_list[5],
